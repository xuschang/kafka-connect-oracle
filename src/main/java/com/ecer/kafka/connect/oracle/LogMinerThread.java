      ...
      while(!this.closed){
        ...
        while(true){
          ...
          try {
            ...
            System.out.println("dbnn " + logMinerSelectSql);
        ...
        while(!this.closed && logMinerData.next()){
          try {
            ...
            log.info("run",filters);
            List filter = Arrays.asList(filters.toUpperCase().replaceAll("\\s\\+","").split(","));
            for(int i = 0;i < filter.size();i++){
              log.info(filter.get(i).toString());
            }
//              if(filter.contains(operation)){
